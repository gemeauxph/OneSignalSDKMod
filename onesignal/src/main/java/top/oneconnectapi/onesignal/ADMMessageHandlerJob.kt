...
class ADMMessageHandlerJob : ADMMessageHandlerJobBase() {
    ...
    override fun onRegistrationError(context: Context?, error: String?) {
        ...
        if ("INVALID_SENDER" == error) OneSignal.Log(
            OneSignal.LOG_LEVEL.ERROR, "Please double check that you have a matching package name (NOTE: Case Sensitive), api_key.txt, and the apk was signed with the same Keystore and Alias.")
