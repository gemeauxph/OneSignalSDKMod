...
import top.oneconnectapi.onesignal.OneSignal.SendTagsError;
import top.oneconnectapi.onesignal.OneSignalStateSynchronizer.UserStateSynchronizerType;
...
import static top.oneconnectapi.onesignal.OSInAppMessageController.IN_APP_MESSAGES_JSON_KEY;
...
abstract class UserStateSynchronizer {
    ...
    final private Queue<OneSignalStateSynchronizer.OSDeviceInfoCompletionHandler> deviceInfoCompletionHandler = new ConcurrentLinkedQueue<>();
        ...
        OneSignalRestClient.putSync("players/" + userId, jsonBody, new OneSignalRestClient.ResponseHandler() {
            ...
            void onFailure(int statusCode, String response, Throwable throwable) {
                ...
                if (jsonBody.has(LANGUAGE))
                    deviceInfoHandlersPerformOnFailure(new OneSignalStateSynchronizer.OSDeviceInfoError(statusCode, response));
            }
