...
import top.oneconnectapi.onesignal.ShadowOneSignalRestClient.REST_METHOD;
...
import static top.oneconnectapi.onesignal.OneSignalPackagePrivateHelper.UserState.PUSH_STATUS_SUBSCRIBED;
import static top.test.onesignal.RestClientValidator.GET_REMOTE_PARAMS_ENDPOINT;
import static top.test.onesignal.TypeAsserts.assertIsUUID;
...
class RestClientAsserts {
   ...
   private static final AnyOf<Integer> ANY_OF_VALID_DEVICE_TYPES = CoreMatchers.anyOf(
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_ANDROID),
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_FIREOS),
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_EMAIL),
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_HUAWEI),
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_SMS)
   );

   private static final AnyOf<Integer> ANY_OF_PUSH_DEVICE_TYPES = CoreMatchers.anyOf(
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_ANDROID),
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_FIREOS),
      CoreMatchers.is(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_HUAWEI)
   );
   ...
   static void assertPlayerCreateAny(ShadowOneSignalRestClient.Request request) throws JSONException {
      assertPlayerCreateMethodAndUrl(request);
      assertValidDeviceType(request.payload);
   }

   static void assertPlayerCreatePush(ShadowOneSignalRestClient.Request request) throws JSONException {
      assertPlayerCreateMethodAndUrl(request);
   ...
   static void assertAndroidPlayerCreateAtIndex(int index) throws JSONException {
      ShadowOneSignalRestClient.Request request = ShadowOneSignalRestClient.requests.get(index);
   ...
   static void assertPlayerCreateNotSubscribedAtIndex(int index) throws JSONException {
      ...
      boolean unsubscribed = request.payload.getInt("notification_types") < UserState.PUSH_STATUS_SUBSCRIBED;
   ...
   static void assertOnFocusAtIndex(int index, @NonNull JSONObject containsPayload) throws JSONException {
      ...
      Assert.assertEquals(ShadowOneSignalRestClient.REST_METHOD.POST, request.method);
   ...
   private static void assertPlayerCreateMethodAndUrl(@NonNull ShadowOneSignalRestClient.Request request) {
      Assert.assertEquals(ShadowOneSignalRestClient.REST_METHOD.POST, request.method);
   ...
   private static void assertDeviceTypeIsAndroid(@NonNull JSONObject payload) throws JSONException {
      assertEquals(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_ANDROID, payload.getInt("device_type"));
   }

   private static void assertDeviceTypeIsAmazon(@NonNull JSONObject payload) throws JSONException {
      assertEquals(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_FIREOS, payload.getInt("device_type"));
   }

   private static void assertDeviceTypeIsHuawei(@NonNull JSONObject payload) throws JSONException {
      assertEquals(OneSignalPackagePrivateHelper.UserState.DEVICE_TYPE_HUAWEI, payload.getInt("device_type"));
   }

   static void assertReportReceivedAtIndex(int index, @NonNull String notificationId, @NonNull JSONObject payload) {
      ShadowOneSignalRestClient.Request request = ShadowOneSignalRestClient.requests.get(index);
      Assert.assertEquals(ShadowOneSignalRestClient.REST_METHOD.PUT, request.method);
   ...
   static void assertRemoteParamsAtIndex(int index) {
      ...
      Assert.assertEquals(ShadowOneSignalRestClient.REST_METHOD.GET, request.method);
   ...
   static void assertOnSessionUrl(String url) {
      ...
      TypeAsserts.assertIsUUID(parts[1]);
   ...
   static void assertRemoteParamsUrl(@NonNull String url) {
      ...
      Assert.assertEquals(RestClientValidator.GET_REMOTE_PARAMS_ENDPOINT, playerIdParts[0]);
      if (playerIdParts.length == 2)
         TypeAsserts.assertIsUUID( playerIdParts[1]);
   ...
   static void assertHasAppId(@NonNull ShadowOneSignalRestClient.Request request) {
      if (request.method == ShadowOneSignalRestClient.REST_METHOD.GET) {
         assertAppIdInUrl(request.url);
         return;
      }

      TypeAsserts.assertIsUUID(request.payload.optString("app_id"));
   }
